...
def _send_desktop_nofity(title, content):
...
def desktop_nofity(title, success_msg, fail_msg):
    """Returns a callback which emits a desktop notification according to execution result."""
    def callback(is_success):
        _send_desktop_nofity(title=title, content=success_msg if is_success else fail_msg)
    return callback
